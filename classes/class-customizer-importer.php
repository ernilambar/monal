...
class Monal_Customizer_Importer {
	/**
	...
	public static function import( $customizer_import_file_path ) {
		...
		Monal_Logger::get_instance()->info( __( 'The customizer import has finished successfully', 'monal' ) );
		return true;
	}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'monal' ),
					$import_file_path
				)
			);
		}
		...
		if ( empty( $raw ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file does not have any content in it. Please make sure to use the correct customizer import file.', 'monal' )
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'monal' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'monal' )
			);
		}

		// Import images.
		if ( apply_filters( 'monal_customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new Monal_Customizer_Option(
					$wp_customize,
					$option_key,
					array(
						'default'    => '',
		...
		$use_wp_customize_save_hooks = apply_filters( 'monal_enable_wp_customize_save_hooks', false );
