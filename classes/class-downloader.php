...
class Monal_Downloader {
	...
	public function download_file( $url, $filename ) {
		...
		if ( is_wp_error( $content ) ) {
			Monal_Logger::get_instance()->error(
		...
		Monal_Logger::get_instance()->error(
			__( 'The file was not able to save to disk, while trying to download it', 'monal' ),
			array(
				'url'      => $url,
				'filename' => $filename,
			)
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'monal' )
			);
		}
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => apply_filters( 'monal_timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'monal' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = apply_filters( 'monal_upload_file_path', trailingslashit( $upload_dir['path'] ) );
		}
	}
